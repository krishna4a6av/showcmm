...
def handle_update() -> None:
    ...
    if confirm.lower() in {"y", "yes"}:
        try:
            ...
            history_files = history.choose_shells(available_shells)

            for shell, file_path in history_files.items():
                history.import_history(shell, file_path)
...
def handle_ai(args) -> None:
    try:
        mode = ("roast" if args.roast else "learn" if args.learn else "workflow" if args.workflow else "level" if args.level else "alias")

        print(ai_module.analyze(mode))
...
def main():
    ...
    ai.add_argument("--roast", action="store_true", help="roast based on top commands")
    ai.add_argument("--learn", action="store_true", help="recommends more prodoctive commands")
    ai.add_argument("--workflow", action="store_true", help="hallucinate some misinformation")
    ai.add_argument("--level", action="store_true", help="hallucinate some misinformation")
    ai.add_argument("--alias", action="store_true", help="hallucinate some misinformation")
